...
\subsection{Principali cambiamenti nella ottava edizione} % TODO: Finire la lista
...
\begin{itemize}
	\item Le meccaniche di spettrificazione sono state modificate. In particolare, è stata introdotta la Successione Spettrale.
\end{itemize}
...
\subsection{Giorni e notti di gioco} % TODO: Decidere a che ora mettere l'alba
...
Le comunicazioni fra GM e giocatori (siano esse verbali, scritte, digitali, telepatiche, o in qualsiasi altra forma) sono assolutamente inviolabili. Lo stesso vale per l'utilizzo dell'area privata dell'interfaccia web, le lettere cartacee consegnate all'inizio della partita, l'account email ufficiale, e l'account del forum.
...
\subsection{Svolgimento del giorno e votazioni} % TODO: Se decidiamo che serve il 50 %+\epsilon dei voti per morire? Va ucciso il sindaco.

L'inizio del giorno è annunciato sul sito web, insieme a tutte le informazioni ottenute da ciascun giocatore in seguito agli avvenimenti della notte appena trascorsa. Ogni giorno, il villaggio convoca un'assemblea per stabilire chi condannare a morte sul rogo. I personaggi vivi hanno il diritto di votare entro le ore 22:00 un abitante del villaggio da condannare a morte. Dopo le ore 22:00, viene pubblicata la lista delle persone votate, insieme al numero di preferenze ricevute da ciascuna persona.

Il voto è ritenuto valido se almeno il 50\% dei vivi ha votato; in caso contrario, nessuno viene condannato. Il personaggio che ha ricevuto più voti di tutti muore. In caso di parità tra due o più personaggi, ne muore uno a caso.
...
Alcuni personaggi hanno un potere attivabile ogni due notti: si intende che un personaggio che utilizza un potere attivabile ogni due notti non può utilizzare alcun potere la notte successiva. %si intende che tale potere può essere usato se e solo se non è stato usato durante la notte precedente: in altre parole, l'unica restrizione è data dal non poterlo usare in due notti consecutive.
Alcuni poteri non possono essere utilizzati la prima notte.
...
Molti dei personaggi non resteranno vivi sino alla fine della partita. Un personaggio può morire in diversi modi: condannato al rogo, sbranato dai Lupi, oppure ucciso da un Assassino.
...
%	\item {\bf Cacciatore} (aura nera). Una sola volta durante l'arco della partita, eccetto la prima notte, il Cacciatore può scegliere un personaggio vivo e puntarlo col fucile. Il Cacciatore uccide il personaggio scelto.
...
%	Questo potere speciale non può essere usato per due notti consecutive sullo stesso personaggio.
	...
	\item {\bf Divinatore} (aura bianca, mistico). Ogni notte, il Divinatore può scegliere un personaggio vivo ed un ruolo, ed effettuare una divinazione su di lui. Scopre se il personaggio scelto ha tale ruolo.
...
%	L'Esorcista può usare il suo potere speciale su sé stesso.
	...
	\item {\bf Investigatore} (aura bianca). Ogni due notti, l'Investigatore può scegliere un personaggio morto e indagare su di esso. Scopre il ruolo di tale personaggio. %che tale personaggio aveva all'inizio della partita.
	...
	\item {\bf Messia} (aura bianca, mistico). Una sola volta durante l'arco della partita, il Messia può scegliere un personaggio morto e resuscitarlo. Quel personaggio ritornerà in vita all'inizio del giorno seguente, riacquistando i suoi poteri speciali. % TODO: Ma se fallisce?
	...
	\item {\bf Spia} (aura bianca). Ogni notte, eccetto la prima, la Spia può scegliere un personaggio vivo e curiosare fra i suoi effetti personali. Scopre a chi era diretto il voto di tale personaggio.

	\item {\bf Stalker} (aura bianca). Ogni due notti, lo Stalker può scegliere un personaggio vivo e pedinarlo. Scopre un eventuale movimento generato da tale personaggio. % Scopre se quel personaggio ha agito su altri personaggi durante la notte, ed in tal caso su chi. Non scopre, però, cosa ha fatto.
...
%	Nel momento in cui il Trasformista ottiene il potere speciale di un altro personaggio, acquisisce anche il ruolo, l'aura e la proprietà di essere mistico del personaggio in questione, ma continua a giocare con la Fazione dei Popolani.
	...
	\item {\bf Voyeur} (aura bianca). Ogni due notti, il Voyeur può scegliere un personaggio vivo e spiarlo. Scopre tutti i movimenti diretti verso la casa del personaggio scelto. %Scopre quali sono gli altri personaggi vivi che durante la notte hanno agito sul personaggio scelto.
...
\begin{itemize}
 ...
 % Se i Lupi tentano di uccidere un Negromante, il loro potere non ha effetto. I Lupi conoscono gli altri Lupi.
	...
	\item {\bf Alcolista} (aura bianca). Ogni notte, l'Alcolista può scegliere un personaggio, vivo o morto, e ubriacarsi vicino casa sua. L'Alcolista riceve una notifica di fallimento.
	
	\item {\bf Assassino} (aura nera). Ogni due notti, eccetto la prima, l'Assassino può scegliere un personaggio vivo e puntare il suo fucile di precisione verso casa sua. Uccide uno degli altri personaggi vivi il cui movimento è diretto verso la casa del personaggio scelto dall'Assassino, scelto in modo casuale. Se il movimento selezionato è stato generato da uno Spettro, non uccide nessuno. %che hanno usato il proprio potere speciale sul personaggio scelto dall'Assassino, selezionato in modo casuale.
...
%	Il Diavolo conosce gli Avvocati, le Fattucchiere, gli Stregoni e gli eventuali altri Diavoli.
	...
	\item {\bf Fattucchiera} (aura bianca, mistico). Ogni notte la Fattucchiera può scegliere un personaggio, vivo o morto, ed un ruolo, e stregarlo. Per quella notte, ruolo, aura, e misticità di tale personaggio risultano come quelle del ruolo scelto. Il potere di tale personaggio non viene influenzato. %Per quella notte, il colore dell'aura del personaggio scelto risulta diverso da quello effettivo.
...
\begin{itemize}
	...
	\item {\bf Negromanti} (aura bianca, mistici). Ogni notte, ciascun Negromante può scegliere uno Spettro e selezionare un potere soprannaturale non assegnato (per la lista dei possibili poteri, vedi oltre). Lo Spettro perde l'eventuale potere soprannaturale precedente e ottiene invece quello selezionato. In ogni momento, ciascun potere può essere assegnato ad al più uno Spettro.
	%Ogni notte, ciascun Negromante può scegliere un personaggio morto e selezionare un potere soprannaturale (per la lista dei possibili poteri, vedere oltre). Se tutti i Negromanti che decidono di usare il proprio potere speciale scelgono lo stesso personaggio e selezionano lo stesso potere soprannaturale, e tale personaggio appartiene alla Fazione dei Popolani, questi diventa uno Spettro e ottiene il potere selezionato. Se almeno due Negromanti scelgono personaggi diversi o selezionano poteri soprannaturali diversi, il loro potere speciale non ha effetto e nessun personaggio viene risvegliato come Spettro.
...
%	Appena viene creato uno Spettro con il potere della Morte, tutti i Negromanti perdono il potere di creare Spettri. 
...
% 	I Negromanti conoscono gli altri Negromanti.
	...
	\item {\bf Fantasma} (aura nera). Il Fantasma non ha alcun potere speciale. Se il Fantasma muore, diventa immediatamente uno Spettro ed ottiene uno dei poteri soprannaturali non assegnati, scelto in modo casuale fra quelli possibili. %Gli viene inoltre comunicata l'identità dei Negromanti, e ai Negromanti viene comunicata l'identità del Fantasma.
...
%	Lo Scrutatore conosce gli Ipnotisti, i Medium e gli eventuali altri Scrutatori.
	...
	\item {\bf Spettri}. Gli Spettri non sono presenti all'inizio della partita: i personaggi morti appartenenti alla fazione dei Popolani possono divenire Spettri se previsto dalla Successione Spettrale.
	
	Prima dell'inizio della partita, i GM decidono segretamente una successione di numeri interi positivi, detta Successione Spettrale. Quando l'$n$-esimo personaggio appartenente alla fazione dei Popolani muore (ovvero prima di lui sono morti $n-1$ Popolani), se $n$ è uno dei numeri della Successione Spettrale, tale personaggio diviene uno Spettro.
 	
 	Nel momento in cui un personaggio diviene uno Spettro, mantiene %il suo ruolo, 
 	la sua aura ed eventualmente la proprietà di essere mistico, ma inizia a giocare per la Fazione dei Negromanti. Gli viene comunicata l'identità dei Negromanti, dai quali gli potrà venire assegnato un potere soprannaturale a partire dalla notte successiva. % che lo hanno risvegliato, e ottiene un potere soprannaturale che può iniziare a usare dalla notte successiva. % Inoltre, perde per sempre gli eventuali poteri speciali che possedeva.
 
	Uno Spettro rimane morto: in particolare non può essere ucciso, e il villaggio non viene informato del suo risveglio.
	 
%	Se uno Spettro viene resuscitato dal Messia, continua a giocare per la Fazione ei Negromanti, ma perde per sempre il suo potere soprannaturale. Non riottiene i poteri speciali eventualmente posseduti prima di essere stato risvegliato come Spettro.
...
\begin{itemize}
	...
	\item {\bf Amnesia}. Ogni notte, lo Spettro può scegliere un personaggio vivo e danneggiarne la memoria. A partire dal giorno successivo, il suo eventuale voto per il rogo viene ignorato: a tutti gli effetti è come se non avesse votato. %ottenebrarne i ricordi. Il giorno successivo, il suo eventuale voto per il rogo viene ignorato: a tutti gli effetti è come se non avesse votato.
	
%	Ciò accade anche se il personaggio scelto è sotto il controllo dell'Ipnotista o dello Spettro con il potere dell'Ipnosi: invece di votare come l'Ipnotista, o come stabilito dallo Spettro con il potere dell'Ipnosi, non vota affatto.
%	Il potere soprannaturale dell'Amnesia non ha effetto sui componenti della Fazione dei Negromanti. 
%	Questo potere non può essere usato per due notti consecutive sullo stesso personaggio.
 
	\item {\bf Confusione}. Ogni notte, lo Spettro può scegliere un personaggio, vivo o morto, e generare confusione attorno a lui. Per quella notte, ruolo, aura, e misticità di tale personaggio risultano come quelle del ruolo scelto. Il potere di tale personaggio non viene influenzato.
...
%	
%	Il potere soprannaturale dell'Ipnosi non ha effetto sui componenti della Fazione dei Negromanti.
%	Se lo Spettro tenta di controllare la mente dell'Ipnotista, il suo potere non ha effetto.
...
%	\item {\bf Duplicazione}. Ogni notte, lo Spettro può scegliere un personaggio vivo e assumerne l'aspetto. Il giorno successivo, il suo voto per il rogo conta doppio. Non risulta, però, che il personaggio abbia votato due volte; semplicemente, il personaggio per cui vota riceve un voto in più.
	...
	\item {\bf Gomma}. Ogni due notti, lo Spettro può scegliere un personaggio vivo, e infondere un'aura di benevolenza intorno a lui. Durante il giorno successivo, tutti i voti diretti verso tale personaggio risultano invece voti nulli.
	
	\item {\bf Illusione}. Ogni due notti, lo Spettro può scegliere un personaggio vivo e dirigere una sua illusione verso la casa di un altro personaggio, vivo o morto. Il movimento del personaggio scelto viene cancellato. Se i due personaggi non coincidono, il movimento viene sostituito da un movimento di tale personaggio verso la casa scelta.
	
%	Ogni due notti, lo Spettro può scegliere un personaggio, vivo o morto, quindi generare un'illusione di un personaggio vivo. L'illusione compare nella casa del personaggio scelto.
...
%	Per il computo dei poteri speciali degli altri personaggi, è come se il personaggio di cui è stata generata l'illusione avesse agito (senza alcun effetto) sul personaggio scelto dallo Spettro (ma se il personaggio di cui è stata generata l'illusione viene sorteggiato dall'Assassino, non viene ucciso). Se tali personaggi coincidono, lo Stalker riceve informazioni come se tale personaggio non avesse agito, mentre gli altri ruoli non sono influenzati in alcun modo.
...
%	\item {\bf Mistificazione}. Ogni notte, lo Spettro può scegliere un personaggio, vivo o morto, e creare un'aura magica nella sua casa. Per quella notte, il personaggio scelto è considerato mistico. 
	...
	\item {\bf Matita}. Ogni notte, lo Spettro può scegliere personaggio vivo, e infondere un'aura di malevolenza intorno a lui. Durante il giorno successivo, tutti i voti nulli risultano invece come diretti verso tale personaggio.
	
	\item {\bf Morte}. Di notte, lo Spettro può scegliere un personaggio vivo e tentare di ucciderlo. Tale personaggio muore.
 
	Lo Spettro perde il potere soprannaturale della Morte. Non gliene possono essere assegnati di nuovi.
	
	% Se lo Spettro tenta di uccidere un Lupo, il suo potere non ha effetto.
...
\begin{itemize}
	...
	\item I componenti della Fazione dei Lupi si conoscono fra loro.
	\item I componenti della Fazione dei Negromanti si conoscono fra loro.
...
Se un personaggio con un potere attivabile ogni due notti (oppure una volta a partita) agisce, a prescindere dal fatto che agisca con successo, non potrà farlo nella notte successiva (o nel resto della partita). %TODO: Ma davvero? :/
